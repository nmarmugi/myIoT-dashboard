...
<template>
    <tr @click="() => measurementsStore.bringToFrontIfExists(sensor.id)" :class="['odd:bg-white even:bg-gray-50 hover:bg-blue-50 transition-colors text-secondaryText', sensor.lastValue && 'cursor-pointer',  sensor.id === measurementsStore.measurements?.[0]?.id && 'border-2 border-yellow-400']">
